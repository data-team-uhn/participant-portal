...
const Consents = () => {
  ...
  const loadData = useCallback(async () => {
    setIsLoading(true)
    const fetchedResponses = await app.service('module-responses').find({ query: { type: FORM_TYPE_ENUM.CONSENT } }) as Paginated<ModulesType>
  ...
  return (
    <PortalMain bgcolor={hasContent ? 'common.inputBackground' : 'common.white'}>
      <SimpleHeader id='consents' titleId='consents.title' title='My Consents' />
      <Box component='section' aria-labelledby='consents' sx={{ maxWidth: '45rem', margin: 'auto' }}>
        {
          isLoading
            ? <Progress />
            : isEmpty(responses)
              ? <EmptyCard
                ...
                headerId='consents.emptyTitle'
                headerMessage='Review your consents'
                bodyId='consents.empty'
                bodyMessage='This is where you can review any consents you have completed.'
              />
